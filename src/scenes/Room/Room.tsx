...
export function Room() {
  ...
  // hide/remove baked neon sign or branding mesh named 'techinz.dev' (fixes old branding showing on mobile)
  useEffect(() => {
    if (!roomGLTF || !roomGLTF.scene) return;

    try {
      // mesh/node in the GLB was named 'techinz.dev'
      const neonNode = roomGLTF.scene.getObjectByName('techinz.dev') || roomGLTF.scene.getObjectByName('techinz');
      if (neonNode) {
        // prefer hiding first to avoid altering parent hierarchy
        neonNode.visible = false;
        // also try removing it from parent so it doesn't participate in raycasts or transforms
        if (neonNode.parent) {
          neonNode.parent.remove(neonNode);
        }
      }
    } catch (err) {
      // swallow any errors here to avoid breaking the room load
      // (for example if parent is null or remove fails)
      // eslint-disable-next-line no-console
      console.warn('Failed to remove neon branding node:', err);
    }
  }, [roomGLTF]);
